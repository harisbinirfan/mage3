 ...
 */
class Tejar_OrbitSlider_Model_Image extends Mage_Catalog_Model_Product_Image
{
   ...
   /**
    ...
    public function setBaseFile($file)
    {
        ...
        $baseDir = Mage::getBaseDir('media') . DS . 'orbitslider' . DS . 'image';
        ...
        if (!$file) {
				...
				// var_dump($baseDir . $configPlaceholder); die;
				if ($isConfigPlaceholder && $this->_fileExists($baseDir . $configPlaceholder)) {
					...
					$skinPlaceholder = "/images/orbitslider/image/placeholder/thumbnail.jpg";
        ...
        $path = array(
			Mage::getBaseDir('media') . DS . 'orbitslider' . DS . 'image',
            'cache',
		...
		if($imagickEnabled){
			...
			switch (strtolower($info['extension'])) {
				...
				case 'gif':
					...
					exec(sprintf($imagickOptionGif, $this->getBaseFile() , $filename), $output, $returnVar);
                    $type = 'gif';
					break;	
			}	 
    ...
    {
        $directory = Mage::getBaseDir('media') . DS.'orbitslider'.DS.'image'.DS.'cache'.DS;
