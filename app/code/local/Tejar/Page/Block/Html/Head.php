    ...
    /**
                                                         ...
                                                         $mergeCallback = null)
       {
           ...
           foreach ($items as $params => $rows) {
               ...
               if ($mergedUrl) {
           $path = parse_url($mergedUrl);
           $path = Mage::getBaseDir() . $path['path'];
                   $html .= sprintf($format, $mergedUrl . "?m=" . filemtime($path), $params);
               } else {
                   foreach ($rows as $src) {
             // $path = parse_url($src);
             // $path = Mage::getBaseDir() . $path['path'];
                       // $html .= sprintf($format, $src . "?m=" . filemtime($path), $params);
