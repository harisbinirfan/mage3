...
class Tejar_Adminhtml_Report_ProductController extends Mage_Adminhtml_Report_ProductController
{
    ...
    public function createdAction()
    {
		...
		$this->_title($this->__('Reports'))->_title($this->__('Products'))->_title($this->__('Created'));
		  ...
		  $this->_initAction()
				->_setActiveMenu('report/product/created')
				->_addBreadcrumb(Mage::helper('reports')->__('Created'), Mage::helper('reports')->__('Created'))
				->_addContent($this->getLayout()->createBlock('tejar_adminhtml/report_product_created'))
				->renderLayout();
    }
    ...
    public function exportCreatedCsvAction()
    {
        ...
        $fileName   = 'products_created_' . $currentDate . '.csv';
        $grid       = $this->getLayout()->createBlock('tejar_adminhtml/report_product_created_grid');
    ...
    {
        $currentDate = Mage::getSingleton('core/date')->date('Y-m-d_H-i-s');
        $fileName   = 'products_created_' . $currentDate . '.xml';
        $grid       = $this->getLayout()->createBlock('tejar_adminhtml/report_product_created_grid');
        $this->_initReportAction($grid);
