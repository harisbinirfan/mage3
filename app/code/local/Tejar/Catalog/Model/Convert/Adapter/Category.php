...
class Tejar_Catalog_Model_Convert_Adapter_Category
    extends Mage_Eav_Model_Convert_Adapter_Entity
{
    const MULTI_DELIMITER   = ' , ';
    const ENTITY            = 'catalog_category_import';
     ...
     * @var string
     */
    protected $_eventPrefix = 'catalog_category_import';
    ...
    public function load()
    {
		...
		$attrFilterArray ['collection_type']    = 'eq';
		$attrFilterArray ['level']   			= 'eq';
		$attrFilterArray ['is_anchor'] 			= 'eq';
		$attrFilterArray ['children_count']   	= 'fromTo';
		$attrFilterArray ['entity_id']		   	= 'fromTo';
		$attrFilterArray ['created_at']		   	= 'dateFromTo';
		$attrFilterArray ['updated_at']		   	= 'dateFromTo';
		$attrFilterArray ['include_in_menu']  	= 'eq';
    ...
    public function getAttribute($code)
    {
        ...
        if ($this->_attributes[$code] instanceof Mage_Catalog_Model_Resource_Eav_Attribute) {
            ...
            if ($applyTo && !in_array($this->getCategoryModel()->getTypeId(), $applyTo)) {
                return false;
            }
        }
    ...
    {
        $fieldset = Mage::getConfig()->getFieldset('catalog_category_dataflow', 'admin');
        ...
        if (!Mage::registry('Object_Cache_Category')) {
            $this->setCategory(Mage::getModel('catalog/category'));
        }
    ...
    {
        return Mage::objects()->load(Mage::registry('Object_Cache_Category'));
    }
            ...
            try {
                ...
                foreach ($collection->getIterator() as $model) {
                    ...
                    if (!$model->getId()) {
                        ...
                        if (0 !== $storeId ) {
                            $data = $model->getData();
                            $default = Mage::getModel('catalog/category');
    ...
    public function saveRow(array $importData)
    {
        ...
        if ($productId) {
            ...
            $productTypes = $this->getCategoryTypes();
            $productAttributeSets = $this->getCategoryAttributeSets();
        ...
        $this->setCategoryTypeInstance($product);
