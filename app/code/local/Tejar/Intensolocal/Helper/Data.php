...
class Tejar_Intensolocal_Helper_Data extends Itactica_Intenso_Helper_Data
{
    ...
    public function getStoreClass() {
		...
		$templateName = strtolower(Mage::getSingleton('core/design_package')->getTheme('template'));
        return ' ' . $storeCode . ' ' . $locale . ' ' . $templateName;
    }
	 ...
	 * @return bool
	 */
	public function getSliderAlternativeImgUrl($product, $width, $height, $isLazyLoading) {
		$store = Mage::app()->getStore();
		if (!Mage::getStoreConfig('intenso/catalog/alternative_image', $store)) {
			return false;
		}
		...
		if ($altImg = $product->load('media_gallery')->getMediaGalleryImages()->getItemByColumnValue($column, $value)) {
			if ($imgFile = $altImg->getFile()) {
				if ($isLazyLoading) {
					return '<img class="alt-image alt-lazyload" src="'.Mage::getDesign()->getSkinUrl('images/clear.png').'" data-src="'.Mage::helper('catalog/image')->init($product, 'small_image', $imgFile)->resize($width, $height).'" width="'.$width.'" height="'.$height.'" alt="'.$product->getName().'">';
				} else {
					return '<img class="alt-image" src="'.Mage::helper('catalog/image')->init($product, 'small_image', $imgFile)->resize($width, $height).'" width="'.$width.'" height="'.$height.'" alt="'.htmlspecialchars($product->getName()).'">';
				}
			}
