...
class Tejar_Sales_Model_Convert_Adapter_Entity
    extends Mage_Dataflow_Model_Convert_Adapter_Abstract
{
    ...
    */
            ...
            switch ($type){
                ...
                case 'dateFromTo':
                    $attr = array(
                        array($keyDB),
						array(
							array('from' => $val['from'], 'to' => $val['to'], 'date' => true)
						)
                    );
                    break;
                case 'datetimeFromTo':
                    $attr = array(
                        array($keyDB),
						array(
							array('from' => isset($val['from']) ? $val['from'] : null,'to' => isset($val['to']) ? $val['to'] : null,'datetime'  => true)
						)
                    );
                    break;
                default:
     ...
     * @return void
     */
    public function setJoinTable($joinAttr)
    {
		if(is_array($joinAttr)){
            $joinArrAttr = array();
			// var_dump($joinAttr['alias']); die;
            $joinArrAttr['table'] = isset($joinAttr['table']) ? $joinAttr['table'] : null;
            $joinArrAttr['alias'] = isset($joinAttr['alias']) ? $joinAttr['alias'] : (isset($joinAttr['table']) ? str_replace('/','_',$joinAttr['table']):null);
            $joinArrAttr['bind'] = isset($joinAttr['bind']) ? $joinAttr['bind'] : null;
            $joinArrAttr['joinType'] = isset($joinAttr['joinType']) ? $joinAttr['joinType'] : null;
            $joinArrAttr['storeId'] = isset($joinAttr['storeId']) ? $joinAttr['storeId'] : $this->getStoreId();
            $this->_joinTable[] = $joinArrAttr;
        }
    ...
    public function load()
    {
        ...
        try {
			...
			if (isset($this->_joinTable) && is_array($this->_joinTable)) {
                foreach ($this->_joinTable as $val) {
					// $collection->getSelect()->
					$collection->getSelect()->join(array($val['alias'] => $val['table']), $val['bind'], $val['field']);
                   // print_r($val);
                    // $collection->joinAttribute(
                        // $val['alias'],
                        // $val['table'],
            ...
            if (is_array($filterQuery)) {
                foreach ($filterQuery as $val) {
                    $collection->addFieldToFilter($val[0],$val[1]);
                }
            }
