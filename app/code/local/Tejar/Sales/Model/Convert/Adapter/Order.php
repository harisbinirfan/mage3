...
class Tejar_Sales_Model_Convert_Adapter_Order
    extends Tejar_Sales_Model_Convert_Adapter_Entity
{
    ...
    public function load()
    {
		...
		$paymentMethod = $this->getVar('filter/payment_method'); //error in key filter addressType
		
		$addressValues = array("both","default_billing","default_shipping");
        
        $attrFilterArray = array();
        $attrFilterArray ['increment_id']              = 'like';
		$attrFilterArray ['customer_email']            = 'like';
        $attrFilterArray ['customer_group_id']         = 'eq';
		$attrFilterArray ['store_id']                  = 'eq';
		$attrFilterArray ['shipping_method']           = 'like';
		$attrFilterArray ['payment_method']            = 'like';
		$attrFilterArray ['firstname']                 = 'like';
        $attrFilterArray ['lastname']                  = 'like';
        $attrFilterArray ['email']                     = 'like';
		$attrFilterArray ['street']                    = 'like';
        $attrFilterArray ['postcode']                  = 'like';
        $attrFilterArray ['region']                    = 'like';
		$attrFilterArray ['city']                      = 'like';
		
		$attrToDb = array();
		
		
        $attrFilterArray ['telephone'] = array(
            'type'  => 'like',
            'bind'  => $addressType
        );
        ...
        if (in_array($addressType,$addressValues)) {
            $qtyFrom = isset($qty['from']) ? (float) $qty['from'] : 0;
            $qtyTo   = isset($qty['to']) ? (float) $qty['to'] : 0;

            $qtyAttr = array();
            $qtyAttr['alias']       = 'address';
            $qtyAttr['table']   = 'sales_flat_order_address';
            $qtyAttr['field']       = array("address.*");
			if ($addressType=='both') {
				$qtyAttr['bind']        = 'address.parent_id=main_table.entity_id';	
			} else {
				$qtyAttr['bind']        = 'address.parent_id=main_table.entity_id AND address.address_type = "'.str_replace("default_","",$addressType).'"';	
			}
            
            $qtyAttr['cond']        = "";
            $qtyAttr['joinType']    = 'left';
			
            $this->setJoinTable($qtyAttr);
        }
		
		
		if($paymentMethod){
			$paymentAttr = array();
            $paymentAttr['alias']       = 'payment';
            $paymentAttr['table']   = 'sales_flat_order_payment';
            $paymentAttr['field']       = "";
			$paymentAttr['bind']        = 'payment.parent_id=main_table.entity_id';	
            $paymentAttr['cond']        = "";
            $paymentAttr['joinType']    = 'inner';
			
            $this->setJoinTable($paymentAttr);
		}
		
		
		if ($qty = $this->getFieldValue($filters, 'payment')) {
